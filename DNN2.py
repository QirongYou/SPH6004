import torch.nn as nn
import torch
import torch.nn.functional as F
import matplotlib.pyplot as plt
import pandas as pd
import numpy as np
from sklearn.preprocessing import LabelEncoder
from sklearn.model_selection import train_test_split
from torch.utils.data import Dataset, DataLoader
from IPython.display import display

torch.manual_seed(1121)
df = pd.read_csv('sph6004_assignment1_data_final.csv')
columns_to_use = [
    'bun_min', 'inr_max', 'gcs_verbal_0.0', 'ph_min', 'admission_age', 'Glucose_Variability', 'gcs_motor_6.0', 'aniongap_max', 'calcium_min', 'weight_admit', 'ph_max', 'glucose_max', 
    'BMI', 'sbp_min', 'albumin_min', 'Oxygenation_Index', 'Acid_Base_Balance_Indicator', 'baseexcess_max', 'dbp_min', 'aniongap_min', 'inr_min', 'ptt_max', 'lactate_min', 'spo2_min', 
    'bilirubin_total_min', 'calcium_max', 'height', 'potassium_max', 'hematocrit_max', 'potassium_max.1', 'temperature_max', 'glucose_max.2', 'sbp_max', 'glucose_min', 'gcs_motor_1.0', 
    'heart_rate_max', 'calcium_min.1', 'abs_lymphocytes_min', 'temperature_min', 'spo2_max', 'potassium_min.1'
]
data=df[columns_to_use]
target=df.iloc[:,0]
 
# 切分训练集和测试集
train_x, test_x, train_y, test_y = train_test_split(data,target,test_size=0.2,random_state=42)

bool_columns = train_x.select_dtypes(include=['bool']).columns
train_x[bool_columns] = train_x[bool_columns].astype(float)
bool_columns_test = test_x.select_dtypes(include=['bool']).columns
test_x[bool_columns_test] = test_x[bool_columns_test].astype(float)

class Data(Dataset):
    def __init__(self):
        self.x=torch.from_numpy(train_x.values)
        self.y=torch.from_numpy(train_y.values).long()
        self.len=self.x.shape[0]
    def __getitem__(self,index):
        return self.x[index], self.y[index]
    def __len__(self):
        return self.len
data_set=Data()


batch_size = 64
def collate_fn(batch):
    inputs, targets = zip(*batch)
    inputs = torch.stack(inputs).float()
    targets = torch.tensor(targets).long()
    return inputs, targets

trainloader = DataLoader(dataset=data_set, batch_size=batch_size, collate_fn=collate_fn)

# D_in, dimension of input layer
# H , dimension of hidden layer
# D_out, dimension of output layer
# Define your neural network class
class Net(nn.Module):
    def __init__(self, D_in, H, H2, D_out):
        super(Net, self).__init__()
        self.linear1 = nn.Linear(D_in, H)
        self.linear2 = nn.Linear(H, H2)
        self.linear3 = nn.Linear(H2, D_out)

    def forward(self, x):
        x = x.float()
        x = F.relu(self.linear1(x))  # ReLU activation
        x = F.relu(self.linear2(x))  # Second layer with ReLU
        x = self.linear3(x)          # Output layer
        return x

# Model, optimizer, and loss function
input_dim = 41
hidden_dim = 20
hidden_dim2 = 10  # Second hidden layer dimension
output_dim = 4
print(input_dim,hidden_dim,hidden_dim2,output_dim)
model = Net(input_dim, hidden_dim, hidden_dim2, output_dim)
print('W:',list(model.parameters())[0].size())
print('b',list(model.parameters())[1].size())
optimizer = torch.optim.Adam(model.parameters(), lr=0.005)  # Adam optimizer with a different learning rate
criterion = nn.CrossEntropyLoss()

# Training process with early stopping
n_epochs = 600
loss_list = []
best_loss = float('inf')
patience = 20  # Patience for early stopping
trigger_times = 0

for epoch in range(n_epochs):
    for x, y in trainloader:
        optimizer.zero_grad()
        z = model(x)
        loss = criterion(z, y)
        loss.backward()
        optimizer.step()
        loss_list.append(loss.item())

    # Early stopping condition
    current_loss = loss.item()
    if current_loss < best_loss:
        best_loss = current_loss
        trigger_times = 0
    else:
        trigger_times += 1
        if trigger_times >= patience:
            print(f"Early stopping at epoch {epoch}")
            break

    if epoch % 100 == 0:
        print(f"Epoch {epoch}, Loss: {current_loss}")
# predict the class and give probablity for each class label
x_val = torch.tensor(test_x.values, dtype=torch.float32)
y_val = torch.from_numpy(test_y.values).float()
z=model(x_val)
display(z)
# choose the predicted class to be the one with maximum probablity
yhat=torch.max(z.data,1)
display(yhat)
# get the indicies
y_pred=yhat.indices.detach().numpy()
display(y_pred)
display(y_val)
from sklearn.metrics import multilabel_confusion_matrix
display(multilabel_confusion_matrix(y_val, y_pred))
# [[[4012 2772]
#   [ 811 2589]]

#  [[8138   18]
#   [2023    5]]

#  [[4929 2093]
#   [1539 1623]]

#  [[8073  517]
#   [1027  567]]]
# 29936/40736=0.73
# Average Precision: 0.415036570167545
# Average Recall: 0.40823192808108766
# Average F1 Score: 0.3728234862977495
# [[[3804 2980]
#   [ 693 2707]]

#  [[8064   92]
#   [1993   35]]

#  [[5126 1896]
#   [1638 1524]]

#  [[8164  426]
#   [1070  524]]]
# 29948/40736=0.73
# Average Precision: 0.4371953558904079
# Average Recall: 0.4060352588926509
# Average F1 Score: 0.3758274017151777
