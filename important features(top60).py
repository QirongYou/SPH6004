...
# Filter feature_names to include only those in importances
filtered_feature_names = [f for f in bst.feature_names if f in importances]

# Convert importances to a list
importances_list = [importances.get(f, 0) for f in filtered_feature_names]

# Check the lengths again
print(len(filtered_feature_names), len(importances_list))

# Combine feature names and importances into a DataFrame
importances_df = pd.DataFrame({'Feature': filtered_feature_names, 'Importance': importances_list})

# Sort the DataFrame by importance
importances_df = importances_df.sort_values('Importance', ascending=False)
importances_df_41=importances_df.head(41)

# Print the top 60 features
print(importances_df_41.iloc[:,0].tolist())

xgb.plot_importance(bst,max_num_features=41,title='重要特征', ylabel='特征')
